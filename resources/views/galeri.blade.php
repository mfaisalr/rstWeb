							...
							<div class="doctor-box-photo">
                                @if ($album->media->count() > 0)
                                    <a href="doctor-page.html">
                                        <img src="{{ asset('storage/' . $album->media->first()->media_path) }}" class="img-fluid" alt="{{ $album->title }}">
                                    </a>
                                @else
                                    <a href="doctor-page.html">
                                        <img src="https://via.placeholder.com/300" class="img-fluid" alt="No Media Available">
                                    </a>
                                @endif
							</div>
							<h5 class="doctor-box-name"><a href="doctor-page.html">{{ $album->title }}</a></h5>
							<div class="doctor-box-position">{{ $album->media->count() }} media</div>
							<div class="doctor-box-text">
								<p>{{ $album->description }}</p>
							</div>
							<div class="doctor-box-bottom">
                                <p>
                                    <a href="{{ route('albums.view', $album->id) }}" class="btn btn-hover-fill js-wrire-review" style="padding: 10px 20px;">
                                        <i class="icon-right-arrow"></i><span>Buka Album</span><i class="icon-right-arrow"></i>
                                    </a>
                                </p>    
                            </div>
						</div>
